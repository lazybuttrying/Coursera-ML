...
**Note that**:
- v는 0으로 초기화 됐기 때문에 v를 만들어나가기 위해 step을 크게 선택하게 됨
- 만약 beta = 0이라면, momentum 없는 일반적이 gradient descent임
- The velocity is initialized with zeros. So the algorithm will take a few iterations to "build up" velocity and start to take bigger steps.
- If $\beta = 0$, then this just becomes standard gradient descent without momentum. 

**How do you choose $\beta$?**

- momentum Beta를 크게 할 수록, 부드럽게 업데이트됨. 왜냐면 과거의 gradients를 더 고려하기 때문. 그러나 beta가 너무 크면 너무 부드럽고 업데이트도 과하게 많이 할 수 있음
- 보통 beta를 0.8~0.999로 함. 0.9 추천
- cost J가 감소하는 beta를 골라야함
...
def update_parameters_with_adam(parameters, grads, v, s, t, learning_rate = 0.01,
    ...
    for l in range(1, L + 1):
        ...
        parameters["W" + str(l)] -= learning_rate * (v_corrected["dW" + str(l)] / (epsilon+np.sqrt(s_corrected["dW" + str(l)])) )
        parameters["b" + str(l)] -= learning_rate * (v_corrected["db" + str(l)] / (epsilon+np.sqrt(s_corrected["db" + str(l)])) )
...
Epoch num이 0->1000일 때 갑자기 훅 줄어듦 ( 더 큰 learning_rate로 설정해도 마찬가지)
...
epoch 수가 적으면 괜찮지만 많을 때 최적화 알고리즘이 업데이트를 그만두는 문제가 발생.  이를 해결하기 위해 fixed interval cheduling을 써보자. epoch이 몇 차례 진행될 때마다 learning_rate를 조금씩 감소시킴
...
accuracy는 서로 비슷하지만 Adam이 빠르다는 사실을 알아차리자!
