...
출력 이미지는 bounding boxes의 리스트들. 5+집단 수(80) 이므로 85개
...
** 해당 박스에 물체 존재 여부와 그 때의 집단별 존재 여부 를 곱해서 확인
#### Visualizing classes
Here's one way to visualize what YOLO is predicting on an image:

- For each of the 19x19 grid cells, find the maximum of the probability scores (taking a max across the 80 classes, one maximum for each of the 5 anchor boxes).
가장 큰 확률를 얻어내기 위해 max() 사용하기
- Color that grid cell according to what object that grid cell considers the most likely.
해당 cell을 산출된 물체의 색깔로 바꿈
...
box가 너무 많이 잡혔을 때 불필요한 상자를 걷어내기 위해 non-max suppression 적용
...
2. For each box, find:
    ...
    **Helpful Hints**
        * For the `axis` parameter of `argmax` and `reduce_max`, if you want to select the **last** axis, one way to do so is to set `axis=-1`.  This is similar to Python array indexing, where you can select the last position of an array using `arrayname[-1]`.
        python에서 -1을 하면 마지막 원소를 가리키게 되는데 numpy에 대해서는 axis라고 하면 됨. 모든 원소에 대해서 max를 보려는 것이 아니라 마지막 컬럼, 즉 집단 별 인식 수치에 대해서만 max()를 적용하기 위해 axis=-1을 꼭 적어주어야 함
...
def yolo_filter_boxes(boxes, box_confidence, box_class_probs, threshold = .6):
    ...
    Arguments:
        boxes -- (𝑏𝑥,𝑏𝑦,𝑏ℎ,𝑏𝑤)  . tensor of shape (19, 19, 5, 4)
        box_confidence -- p,  tensor of shape (19, 19, 5, 1)
        box_class_probs --    (𝑐1,𝑐2,...𝑐80)  ,tensor of shape (19, 19, 5, 80)
...
    """
    ...
    ##(≈ 7 lines)
    if box1_x2 < box2_x1 or box1_y2 < box2_y1:
        return 0.0
