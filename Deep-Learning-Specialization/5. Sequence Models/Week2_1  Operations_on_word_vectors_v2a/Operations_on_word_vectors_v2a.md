...
for w in name_list:
    print (w, cosine_similarity(word_to_vec_map[w], g)) # g는 여성과 남성 단어의 차이
    # g 와 여성 이름이 만나면 양수 값의 cosine similarity 산출
```
...
for w in word_list:
    ...
    # 립스틱은 양수로 여성적인, 총은 음수로 남성적인 이라고 나옴 ㅠㅠ
```
...
중립화 : 만약 50 차원의 word embedding을 사용한다면 50차원을 두 부분을 나눔. bias-direction 하나 ($g$), 그 외의 non-bias 49개 ($g_{\perp}$ ).
...
편견 단어 $e_{receptionist}$ / 편견 X 단어 $e_{receptionist}^{debiased}$. 
...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = g*(np.dot(e,g)/ np.sum(g**2))
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    w1, w2 = pair[0], pair[1]
    #e_w1, e_w2 = neutralize(w1, bias_axis, word_to_vec_map), neutralize(w2, bias_axis, word_to_vec_map)
